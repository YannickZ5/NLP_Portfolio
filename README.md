...
Zuerst sollte der Code heruntergeladen und extrahiert werden. Anschließend muss in den Main-Order gewechselt werden. Um den Code auszuführen, sollte im Vorfeld eine virtuelle Umgebung erzeugt werden. 

Diese lässt sich über den Befehl `python -m venv nlp_env` realisieren. 

Anschließend muss via `nlp_env\Scripts\activate.bat` die Umgebung gestartet werden.

Dann können mit Hilfe des Befehls `pip install -r requirements.txt` alle nötigen Pakete installiert werden.
